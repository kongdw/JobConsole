        ...
        <div class="row">
            ...
            <div id="breadcrumbs">
                ...
                <div class="breadcrumb-button">
                    <span class="breadcrumb-label"><i class="icon-edit"></i> 修改作业</span>
                    <span class="breadcrumb-arrow"><span></span></span>
                </div>
            </div>
        </div>
    ...
    <div class="container">
        <div class="widget-box">
            <div class="widget-box-header">
                <div class="col-sm-2">
                    <div class="title"><i class="icon-edit">&nbsp;</i>修改作业</div>
                </div>
            </div>
            <div class="widget-box-content" style="padding:15px;">
                ...
                <form class="form-horizontal" role="form" action="/job/update" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <input  id="txtJobName" type="TEXT" class="form-control" name="jobName" placeholder="作业名称"
                                   value="${job.name}" required readonly=”readonly”>

                            <%--<p class="help-block">为作业起一个方便辨识的名称。</p>--%>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <select  id="selLayer"  class="form-control" name="layerId" required="required" readonly=”readonly”>
                                <option value="">请选择...</option>
                                <c:forEach items="${layerList}" var="layer">
                                    <c:if test="${layer.id  eq job.layer.id}">
                                        <option selected value="${job.layer.id}">${job.layer.name}</option>
                                    </c:if>
                                    <c:if test="${layer.id ne job.layer.id}">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <select  id="selModeCode"  name="modeCode" class="form-control"
                                    required="required">
                                <option value="">请选择...</option>
                                <c:forEach items="${modeList}" var="c">
                                    <c:if test="${c.code  eq job.mode.code}">
                                        <option selected value="${job.mode.code}">${job.mode.description}</option>
                                    </c:if>
                                    <c:if test="${c.code  ne job.mode.code}">
                    ...
                    <c:if test="${sqlType eq 'i'}">
                        <div class="form-group">
                            ...
                            <div class="col-sm-5 checkbox">
                                <input id="truncateFlag" name="truncateFlag"
                                <c:if test="${job.truncateFlag}"> checked </c:if> type="checkbox">
                            </div>
                        </div>
                    </c:if>
                    ...
                    <c:if test="${sqlType eq 'i'}">
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select  id="selsourceDatabaseId"  name="sourceDatabaseId" class="form-control">
                                    <option value="">请选择...</option>
                                    <c:forEach items="${databaseList}" var="c">
                                        <c:if test="${c.id  eq job.sourceDatabase.id}">
                                            <option selected
                                                    value="${job.sourceDatabase.id}">${job.sourceDatabase.name}(${c.username})
                                            </option>
                                        </c:if>
                                        <c:if test="${c.id  ne job.sourceDatabase.id}">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <select id="targetDatabaseId" class="form-control" name="targetDatabaseId">
                                <option value="">请选择...</option>
                                <c:forEach items="${databaseList}" var="c">
                                    <c:if test="${c.id  eq job.targetDatabase.id}">
                                        <option selected
                                                value="${job.targetDatabase.id}">${job.targetDatabase.name}(${c.username})
                                        </option>
                                    </c:if>
                                    <c:if test="${c.id  ne job.targetDatabase.id}">
                                        <option value="${c.id}">${c.name}(${c.username})</option>
                                    </c:if>
                                </c:forEach>
                            </select>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var SqlScriptVal = $("#sqlScript").val();
        editor.getSession().setValue(SqlScriptVal);

        /*
        var modeType = $('#modeCode').find('option:selected').val();
        if (modeType == 'TASK_LIST' || modeType == 'TIME_PERIOD') {
            ...
            $('#truncateFlag').attr("disabled", true);
            $('#sourceDatabaseId').attr("disabled", true);
        } else {
            $('#splitValue').attr("disabled", true);
        }
        */
    ...
    function modeChange(obj) {
        ...
        if (modeType == 'TASK_LIST' || modeType == 'TIME_PERIOD') {
            ...
            $('#splitValue').attr("disabled", "disabled");
            $('#targetTable').attr("disabled", "disabled");
            $('#sourceDatabaseId').attr("disabled", "disabled");
        } else {
            $('#splitValue').attr("disabled", "disabled");
        }
    }*/
    function init() {
        $('#splitValue').removeAttr("disabled");
        $('#targetTable').removeAttr("disabled");
        $('#sourceDatabaseId').removeAttr("disabled");
        $('#truncateFlag').removeAttr("disabled");
    }
